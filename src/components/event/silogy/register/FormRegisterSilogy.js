...
import {useUserToken, user as userData, signOut} from "../../../../helper/Auth";
...
const FormRegisterSilogy = (props) => {
    ...
    const sendRegister = () => {
        ...
        const dataFull = {
            ...
            'proof_himsika': data.proof_himsika,
            'proof_edufair': data.proof_edufair,
            'type': data.type,
        ...
        axios.post(`${process.env.REACT_APP_API_URL}/register/silogy/2023`, dataFull, {
            headers: {
    ...
    return (
        <form autoComplete="off" onSubmit={validateData}>
            ...
            <div className="mb-6">
                <label htmlFor="instansi" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                    {type != 'kti' ? 'Nama Ketua Tim'  : 'Nama Peserta'}
                </label>
                <input type="text" id="instansi"
                       className="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 disabled:cursor-not-allowed disabled:bg-gray-100"
                       placeholder={type != 'kti' ? 'Nama Ketua Tim' : 'Nama Peserta'}
                       required
                       onChange={(e) => setData({...data, team_lead: e.target.value})}
                       disabled={isLoading}
                />
            </div>
            {type != 'kti' && (
...
export default FormRegisterSilogy;